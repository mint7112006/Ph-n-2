...
Result solveOne(int n, int m, const vector<string>& inputCodes, vector<ull>& codes) {
    ...
    if (!res.validSize) {
        res.reason = "Kich thuoc tap ma khong phu hop.";
        return res;
    }
...
void printFull(
    ...
    int n,
    int m,
    const vector<string>& inputCodes,
    const vector<ull>& codes,
    const Result& res
) {
    cout << "========== TEST CASE " << tc << " ==========\n";
...
int main() {
    ...
    int T;
    cout << "Nhap so luong test case T: " << flush;
    cin >> T;
    ...
    for (int tc = 1; tc <= T; ++tc) {
        ...
        cout << "\n========== TEST CASE " << tc << " ==========\n";
        cout << "Chon che do xuat ket qua:\n";
        cout << "1. Chi in YES/NO\n";
        cout << "2. In loi giai tu luan day du\n";
        cout << "Nhap lua chon: " << flush;
        cin >> mode;

        cout << "\nNhap do dai tu ma n va so luong tu ma m: " << flush;
        cin >> n >> m;

        vector<string> inputCodes(m);

        cout << "\nNhap " << m << " tu ma nhi phan, moi tu ma co do dai " << n << ":\n" << flush;
        for (int i = 0; i < m; ++i) {
            cout << "Tu ma thu " << i + 1 << ": " << flush;
            cin >> inputCodes[i];
        }
